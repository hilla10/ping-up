...
const sendNewConnectionRequestReminder = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run('send-connection-request-email', async () => {
      ...
      const body = ` <div style="font-family: Arial, Helvetica, sans-serif; padding: 20px; background-color: #f9f9f9; color: #333;">
  <div style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 8px; padding: 30px; box-shadow: 0 0 10px rgba(0,0,0,0.05);">
    ...
    <h2 style="color: #111;">Hi ${connection.to_user_id.full_name},</h2>

    <p style="font-size: 16px; line-height: 1.5;">
      You have a new connection request from 
      <strong>${connection.from_user_id.full_name}</strong> 
      - <span style="color: #10b981;">@${connection.from_user_id.username}</span>.
    </p>
...
const sendNotificationOfUnseenMessages = inngest.createFunction(
  { id: 'send-unseen-messages-notification' },
  {
    ...
    tz: 'Africa/Addis_Ababa', // Ethiopia time zone
  }, // Every Day 9 Am
